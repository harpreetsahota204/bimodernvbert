...
class BiModernVBertConfig(fout.TorchImageModelConfig):
    """
    Config class for BiModernVBert.
    
    BiModernVBert is a vision-language model that generates 768-dimensional embeddings
    for both images and text in a shared vector space, enabling visual document retrieval
    and zero-shot classification.
    
    Args:
        model_path (str): HuggingFace model ID. Default: "ModernVBERT/bimodernvbert"
    ...
    def __init__(self, d):
        ...
        # BiModernVBert-specific configuration
        self.model_path = self.parse_string(d, "model_path", default="ModernVBERT/bimodernvbert")
        self.text_prompt = self.parse_string(d, "text_prompt", default="")


class BiModernVBertModel(fout.TorchImageModel, fom.PromptMixin):
    """
    BiModernVBert model for document understanding and retrieval.
    
    This model generates 768-dimensional single-vector embeddings for both images
    and text, enabling:
    - Similarity search and retrieval
    - Zero-shot classification
    - Embeddings visualization
    
    Unlike multi-vector models (ColPali, Jina v4), BiModernVBert uses simple
    cosine similarity scoring on single vectors, making it more efficient for
    ...
    def __init__(self, config):
        ...
        Args:
            config: A BiModernVBertConfig instance containing model parameters
        """
        ...
        self._last_computed_embeddings = None  # Last computed 768-dim embeddings
    ...
    def _load_model(self, config):
        """Load BiModernVBert model and processor from HuggingFace.
        
        Args:
            config: BiModernVBertConfig instance containing model parameters
    ...
    def _get_text_features(self):
        ...
        Returns:
            torch.Tensor: Text embeddings with shape (num_classes, 768)
        """
    ...
    def _embed_prompts_internal(self, prompts):
        ...
        Returns:
            torch.Tensor: Text embeddings with shape (batch, 768)
        """
    ...
    def embed_prompt(self, prompt):
        """Embed a single text prompt to 768-dim vector for retrieval.
        
        Args:
            prompt: Text prompt to embed
            
        Returns:
            numpy array: 768-dim embedding vector
        """
    ...
    def embed_prompts(self, prompts):
        """Embed multiple text prompts to 768-dim vectors for retrieval.
        
        Args:
            prompts: List of text prompts to embed
            
        Returns:
            numpy array: 768-dim embeddings with shape (batch, 768)
        """
    ...
    def embed_images(self, imgs):
        """Embed images to 768-dim vectors for retrieval/similarity search.
    ...
    def embed_all(self, imgs):
        ...
        Returns:
            numpy array: 768-dim embeddings
        """
        return self.embed_images(imgs)
    
    def get_embeddings(self):
        """Get the last computed 768-dim embeddings.
        
        Returns:
            numpy array: The last computed embeddings with shape (batch, 768)
        """
    ...
    def _get_class_logits(self, text_features, image_features):
        ...
        Args:
            text_features: Text embeddings (num_classes, 768)
            image_features: Image embeddings (num_images, 768)
